...
function errorFired(e){
    ...
    console.log(distinctMessageMap);
...
function onSaveFired(fileName){
    ...
    if(state=="ERROR_CHECKING"){
        ...
        $("#ai_inspector_next_go").removeAttr("disabled");
        return;
    }else if(state=="CLEAN"){
...
function startErrorCheckingThread(){
    ...
    $("#ai_inspector_next_go").attr("disabled", "disabled");
    $("#ai_inspector_nextContainer").hide();
    $("#ai_inspector_animation").show();
    $("#ai_inspector_iframe").attr('src',v.app.url+"&inspectionMode=true");
     v.errors=[];
    currentErrorCheckingThread=setTimeout(function(){
        //done error checking, show results
        if(v.errors.length>0){//error found
            state="ERROR_FIXING";
            $("#ai_inspector_topContainer").show();
            $("#ai_inspector_go").removeAttr("disabled");
            var someTabTriggerEl = document.querySelector('#ai_inspectorTab')
            var tab = new bootstrap.Tab(someTabTriggerEl);
            tab.show();
            $("#ai_inspector_nextContainer").show();
        }else{
            state="CLEAN";
        }
...
function getHtml(){
    return `
        <center>
            <img src="eyes-joypixels.gif" style="display: none; width: 50px" id="ai_inspector_animation"/>
        </center>
        <center class="mt-2" id="ai_inspector_topContainer" style="display: none">
            <button id="ai_inspector_go" v-on:click="askAI();" style="width: 200px">
                <span class="bi bi-bug-fill"></span>
            </button>
            <p style="color:red">AI 幫幫忙</p>
        </center>
        <center class="mt-2" id="ai_inspector_nextContainer" style="display: none">
            <button id="ai_inspector_next_go" v-on:click="nextRound();" style="width: 200px" disabled="disabled">
                <span class="bi bi-arrow-right-square-fill"></span>
            </button>
            <p style="color:red">我改好了，再幫我看看</p>
        </center>
        <div>
            <iframe v-bind:src="app.url+'&inspectionMode=true'" style="display: none" id="ai_inspector_iframe"></iframe>
            <ul class="list-group">
                <li class="list-group-item" v-for="e in errors">{{e.simpleFileName}} {{e.lineno}}  {{e.error.message}}</li>
            </ul>
        </div>
        <!--button id="ai_inspector_clear" class="btn btn-link" onclick="$('#ai_inspector_reply').html('');" style="display: none">清空</button-->
        <div id="ai_inspector_reply" style="width: 100%">
...
function resetErrorQueue(){
    $("#ai_inspector_animation").hide();
    $("#ai_inspector_iframe").attr('src','about:blank');
    v.errors=[];
    distinctMessageMap={};
    $("#ai_inspector_topContainer").hide();
    $(".list-group").show();
}
...
function init(){
    v=new Vue({
        ...
        methods: {
            askAI: function(){
                $(".list-group").hide();
                $('#ai_inspector_reply').html('');
                $("#ai_inspector_go").attr("disabled", "true");
                ...
                $.ajax("/ai/codeCoach", {
                    ...
                    success: function(d){
                        ...
                        /*$("#ai_inspector_clear").show();
                        $("#ai_inspector_reply").html(d.message);
                        resetErrorQueue();*/
                    },
                    error: function(){
                        resetErrorQueue();
...
//                        checkingErrors=false;
                    ...
                    try{
                        ...
                        for (let i = 0; i < d.length; i++) {
                            ...
                            $("#ai_inspector_reply").append(cell);
